...
5. **虚拟化**
   ...
   - 虚拟化CPU :  机制 + 策略
     ...
     - 采用**多级反馈队列**的调度方式, 在**同级优先度**的情况下采用**轮转**策略, 和**上下文切换**的机制, 完成进程的调度
   - 虚拟化内存
     - OS 在单一的物理内存上为多个进程构建一个私有的, 很大的**地址空间**抽象;
       - 用户程序生成的每个地址(能够看见的)都是虚拟地址;
     - 使用最少最近使用**LRU缓存替换**策略, 使得总体TLB**未命中**的比例最小;采用**分页**的机制, 快速的从虚拟地址中得到物理地址
...
1. 为什么要虚拟化内存?
   + OS让没一个程序员觉得, 它是有用一个很大的连续地址存储空间, 用来存放其代码和数据;
...
1. 内存分为哪几种类型?
   ...
   + 堆(heap)内存 : 它的申请是程序员**显示**调用**库函数**(不是系统调用);
     ...
     char *string = (char *)malloc(sizeof(strlen(string)) + 1); 字符串一定要加1, 存在'\0'
     ```
   ```
