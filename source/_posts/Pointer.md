...
2. 指针概念
   ...
   int *pi = &i;	/*全局变量的赋值：右边必须是常量表达式*/
   				/* 定义一个指向int型的指针变量pi,并用i的地址来初始化pi  */
   char c;
   char *pc = &c;	/*32位的虚拟机，他们的内存单元都是4 byte*/
   ```
   
   +  指针变量 pi 和pc 指向不同类型的指针变量，但内存单元都占4 byte；
     + 32 bit 的平台：占4byte；
     + 64 bit 的平台：占8byte；
...
1. 代码方式
   ...
   ```
   const int *a;
   or int const *a;	/* 将 const 后面的(*a)看成一个整体，这个整体不允许修改的 */
   ```
...
2. 函数 type  f ( ) : 这个()是函数的调用操作符;type 是返回值的类型

   ```
   int *f();	// 首先结合 f()-函数；再结合 * -- 函数返回值是指向int型的指针
   int (*f)();	// 首先结合 (*f)-聚合为一个指针；再结合()--(*f)()是一个函数指针；函数返回整形值
   int *(*f)(); // (*f)()指针函数； [(*f)()]组合 * 后称为一个指针，指向int型
   ```
...
3. 数组 type g[ ]
   ...
   ```
   int f[]; // 数组
   int *f[];	// 首先结合 f[] 成为数组，元素类型是指向 int 型的指针；
   int (*f[])();	// f[]是一个数组；数组元素类型是函数指针；返回值是整形值	
   ```


### 函数指针 --- [回调函数的实际应用](https://github.com/quronghui/DataStructAndAlogrithmCode/blob/master/SwordOffer/06_List/callback_use.c)

1. 函数指针的初始化

   ```
   int (*pf)(int value) = &f;	// 函数指针的定义和初始化，f也是函数；
   ```
...
2. 回调函数的应用
   ...
   + 功能函数能执行**不同类型**的工作，或者执行只能右函数调用者定义的工作；
   + 许多窗口系统，使用回调函数连接多个动作；
