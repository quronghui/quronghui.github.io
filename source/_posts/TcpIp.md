...
3. 引入子网掩码的IP地址
   ...
   + IP地址本身 + 表示网络的子网掩码
   + CIDR: 将多个C类地址聚合成一个大的地址, 减少一位网络地址,增加一位主机地址;
     + 路由聚合, 减小路由控制表
...
1.  地址格式含义
   ...
   | --------------------------------- | ---------------------- | ------------------------------------------------------------ |
   | 版本号                            | 0-3(4bit)              | 4 = 0100                                                     |
   | 首部长度 : 首部字节数 5*4 =20     | 4-7(4bit)              | 5 = 0101                                                     |
...
| **32bit一组**                                       |              |                                    |
| 有效负载长度: 能包含的数据包长度 **不包含首部长度** | 0-15         | 2^15=65535 bytes: 不包含首部       |
...
1. ARP协议:  -- IPv4   IPv6 (ICMPv6)
   + 以目标IP地址地址为线索, 定位**下一个**接收数据包的网络设备对应的MAC地址;
   + 下一个接受数据包的: 目的主机, 或者路由器
2. ARP工作机制, 如何获得MAC?
...
3. RARP : Reverse 
   + 从MAC地址定位字节的IP地址;
   + 例子: 嵌入式设备接入网络得到IP地址
4. DHCP: dynamic host configuration protocal
   + 通过DHCP自动分配获取IP地址;
...
3. DHCP client server具备的 功能:
   + server: 在分配IP之前, 发送ICMP回送请求包
   + client : 正对获取的IP地址, 发送ARP请求包;
4. DHCP 中继代理
   + 作为客户端和服务器之间的桥梁;
   + 避免每一个客户端都需要与服务器直连
...
2. 路由控制的定义?
   + 根据目的**IP**地址, 根据构成的**路由控制转发表**, 按照**正确的方向**转发数据;
3. 路由的分类
...
1. RIP的特点
   ...
   + 不予子网掩码进行交互;
   + 链路出问题时, 不确定是哪一个链路出问题就进行路由控制, 稳定周期很长
2. RIP如何生成路由控制信息表?
   + 首先: 路由器每30秒广播已知的路由信息; 等待6次后没有反馈则断开网络;
   + 然后: 从距离向量数据库中, 抽出距离短的作为新的控制向量表;
3. 路由变更的时候,如何更新?
   + 定期进行广播, 下一个路由在原基础上加1后, 传给下一个路由; ( 要传回发送路由, 不然6个周期认为断开 )
4. 无限计数的问题?
   + 方法一: 定义最长条跳数为16, 超过后还未达到, 认为链路处于断开状态;
   + 方法二: 水平分割, 路由器不在把收到的信息返回给发送端;
5. 环路问题的解决?
   + 毒性逆转: 当链路中有断开时, 不是不在发送信息, 而是将这个无法传播的消息发送出去(发送距离为16的消息)
   + 触发更新: 当链路变化时, 不再是定期广播, 而是立刻发送, 使得链路尽快收敛;
6. RIP2
   + 采用多播的形式;
...
1. OSPF 的特点?
   ...
   + 链路状态算法型的路由协议: 以最小权重代价为选择;
   + 每个路由器都包含整个网络的拓扑结构, 得到最优路劲;
   + 与中心路由交互得到控制信息;

2. OSPF 如何得到路由控制表?

   + 首先发送HELLO协议: LAN每10秒发送hello包, 却是是否处于连接状态
...
2. 段数据发送 , 如何增加**吞吐量**?
   ...
   + 滑动窗口: 将n个段一起发送, 收到某个段的应答是, 互动窗口下移;
...
3. 窗口控制和重发控制?
   ...
   + 窗口大小控制 ( 数据流控制 )
     ...
     + b) 拥塞控制:
       ...
       + 遇到超时重发时, 设置满期慢启动阀值, 将拥塞窗口减小为一半;
   + 重发控制: 明白重发的两种情况
     + ACK确认应答丢失, 此时数据收到; 
       + 无需重发, 只需要确认下一个序列号就行
     + 数据包丢失, server 3次发送重复确认
...
1. TCP首部: 20bytes
   ...
   | 保留位                  | 2^4      | 0 还未用                                                     |
   | 控制大小                | 2^8      | 每一位使用的时候设置为1<br />ACK = 1 : 用于确认时一直为1<br />SYN = 1: 确认建立连接<br />FIN = 1: 断开连接 |
...
1. **校验和**的计算需要**UDP伪首部**
   + UDP的位首部概念: 标识一个TCP/IP通信需要5各部分, udp中只包含两个端口号, 其余的在TCP中;
...
1. FTP传输的需求
   + 先在远程登录的情况下, 计算机之间进行文件传输的协议;
2. FTP工作机制
   + 建立两条TCP连接; TCP21
   + 数据传输的TCP 和 控制的TCP按照相反的方向建立连接
3. FTP连接断开机制
